...
_Downshift_ has been affected by breaking changes in v8, so check out the
...
The component can be used with _React Native_ as well. The HTML elements and
styles are replaced with React Native equivalents, but the _Downshift_ usage is
exactly the same as on React web.

<ThemedSnack snackId="@silviuaavram/downshift-in-react-native" />
